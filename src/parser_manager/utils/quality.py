...
def score_quality(text: str, semantic_blocks: list[dict]) -> dict:
    ...
    non_printable_count = sum(1 for ch in text if ch not in "\n\r\t" and not ch.isprintable())
    replacement_char_count = text.count("�")
    weird_symbols_count = len(re.findall(r"[□■▪•◊¤§¶]+", text))

    noise_ratio = _safe_ratio(non_printable_count + weird_symbols_count, max(total_chars, 1))
    ...
    table_blocks = sum(1 for b in semantic_blocks or [] if b.get("element_type") == "table")
    table_coverage = _safe_ratio(table_blocks, max(block_count, 1)) if block_count else 0.0
